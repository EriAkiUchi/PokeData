...
TGrafo& readFile(std::string fileName) {
	...
	std::getline(file, line); // Linha para receber a quantidade de vertices 
	...
	if (file.is_open()) {
		while (i < numVertic) {
			...
			verticeNome.erase(remove(verticeNome.begin(), verticeNome.end(), ' '), verticeNome.end()); // Remove possiveis espacos da string
